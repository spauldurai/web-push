...
1. You must have the [Firebase CLI](https://firebase.google.com/docs/cli/) installed. If you don't have it install it with `npm install -g firebase-tools` and then configure it with `firebase login`. if it doesn't work try `firebase login:ci --interactive`
1. On the command line run `firebase use --add` and select the Firebase project you have created. if it doesn't work try `firebase use --add <project id> --token "TOKEN"`
1. On the command line run `firebase serve -p 8081` using the Firebase CLI tool to launch a local server. if it doesn't work try `firebase serve -p 8081  --token "TOKEN"`
